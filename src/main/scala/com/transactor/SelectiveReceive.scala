...
//Solve trouble with selectivity overfloww
object SelectiveReceive:
  ...
  private def intercept[T: ClassTag](
    ...
    Behaviors.receive { case (ctx, message) =>
        val transcatorHandledMessage: Behavior[T] 
            = Behavior.interpretMessage(started, ctx, message)   
        if Behavior.isUnhandled(transcatorHandledMessage) then
            buffer.stash(message)
            Behaviors.same
        else 
            val patchedTransactorState = Behavior.canonicalize(
                transcatorHandledMessage,
                started,
                ctx
            )
            buffer.unstashAll(apply(bufferSize, patchedTransactorState))
    }
